...
<form id="frm" name="frm" method="post" action="/bankJSPProject/deposittrust/deposittrustnew/savingLastStep.do" class="ajax-form" data-no-ajax="true" data-target="#RECV_NAME">
...
${signDate}개월
<input type="hidden" name="signDate" value="${signDate}" /> <!-- 가입기간 -->
</td></tr>
<tr><th scope='row'>자동이체 신청여부</th><td colspan='3'>
${autoTransYorN}
<input type="hidden" name="autoTransYorN" value="${autoTransYorN}" /> <!-- 자동이체신청여부 -->
</td></tr>
...
<input type="hidden" name="iza" value="만기일시지급식"/> <!-- 이자지급방법 -->
</td></tr>
<tr><th scope='row'>비과세선택</th><td colspan='3'>
${tax} 
<input type="hidden" name="tax" value="${tax}" /> <!-- 비과세 -->
<tr><th scope='row'>개인정보수집 동의여부</th><td colspan='3'>
${dntn_inf_ofr_agr_yn} 
<input type="hidden" name="dntn_inf_ofr_agr_yn" value="${dntn_inf_ofr_agr_yn}" /> <!-- 개인정보수집동의여부 -->
</td></tr>
...
<input type="hidden" name="sms" value="${sms}" /> <!-- sms 만기알림 -->
	...
	<input type="submit" value="확인" style="border: solid 3px blue;background: blue;color: white;font-family: '맑은고딕';font-style: italic; " /><b>을 누르면 가입이 완료 됩니다.</b>
	<input type="button" value="처음 화면으로" style="border: solid 3px blue;background: blue;color: white;font-family: '맑은고딕';font-style: italic; float: right;" onclick="location.href='/bankJSPProject/main.jsp'"/>
</form>
</head>
</div>
</div>
