...
public class ProductUpdateService {
	...
	public List<G_DTO> g_itemSearch(String item_no) {
		G_DAO dao = G_DAO.getInstance();
		
		try(Connection conn = ConnectionProvider.getConnection()){
			List<G_DTO> list = dao.selectByGItemNo(conn, item_no);
			return list;
	...
	public List<S_DTO> s_itemSearch(String item_no) {
		S_DAO dao = S_DAO.getInstance();
		
		try(Connection conn = ConnectionProvider.getConnection()){
			List<S_DTO> list = dao.selectBySItemNo(conn, item_no);
			return list;
		} catch (SQLException | NamingException e) {
	...
	public List<Y_DTO> y_itemSearch(String item_no) {
		Y_DAO dao = Y_DAO.getInstance();
		try(Connection conn = ConnectionProvider.getConnection()){
			List<Y_DTO> list = dao.selectByYItemNo(conn, item_no);
			return list;
		} catch (SQLException | NamingException e) {
			System.out.println("ProductUpdateService s_itemSearch 예외 발생");
			throw new RuntimeException(e);
